...
### (SOME DATA BASED ON LARGE (*100*) INSTANCES OF RANDOM CONTROLS)
...
set ylabel "P(k)" 1,0 font "Helvetica,25"
set xtics 5; set mxtics 5
set ytics 5; set mytics 5
plot [:max_degree+k_breath][:1.1] 'degree_dist_norm_stat.txt' u 1:2 title 'Degree Distribution' w linesp 7
replot
...
##############################################################
### PLOT03: Degree Correlations: All nodes (LAST INSTANCE) and     ###
### Avg. of degrees Neighboring Nodes for a degree (ALL INSTANCES) ###
### (PARTIALLY BASED ON MANY INSTANCES) ###
...
plot [:max_degree+k_breath][:] 'degree_corr.txt' u 1:2 notitle w points 6,\
	 'degree_corr_avg_stat.txt' u 1:2 notitle w points 7
replot
...
##############################################################
### PLOT04: CC x k (LAST INSTANCE) and CC(k) x k (ALL INSTANCES) ###
### (PARTIALLY BASED ON MANY INSTANCES) ###
...
plot [:max_degree+k_breath][0:1] 'cc_degree.txt' u 2:1 notitle w points 6,\
	 'cc_degree_corr_stat.txt' u 1:2 notitle w points 7
replot
...
plot [:][:1.1] 'cc_dist_norm_stat.txt' u 1:2 notitle w linesp 7
...
plot [:diameter+dia_breath][:1.1] 'shortest_path_dist_norm_stat.txt' u 1:2 notitle w linesp 7
